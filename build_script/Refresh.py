...
try:
    ...
    sys.exit(
        "[error] Please pip install aliyun-python-sdk-cdn and aliyun-python-sdk-core and logging，please install now......"
    )
...
class Envariable(object):
    ...
    def set_acs_client():
        Envariable.CLI = AcsClient(Envariable.get_ak(), Envariable.get_sk(),
                                   Envariable.REGION)
...
class InitHandler(object):
    def __init__(self, ak, sk, region):
        try:
            self.client = AcsClient(self, Envariable.get_ak(),
                                    Envariable.get_sk(), Envariable.REGION)
        except Exception:
...
class BaseCheck(object):
    ...
    def printQuota(self):
        ...
        try:
            ...
            quotaResp = json.loads(
                Envariable.get_acs_client().do_action_with_exception(quotas))
        ...
        if Envariable.TASK_TYPE:
            if Envariable.TASK_TYPE == 'push':
                if self.lines > int(quotaResp['PreloadRemain']):
                    sys.exit(
                        "\n[error]：PreloadRemain is not enough {0}".format(
                            quotaResp['PreloadRemain']))
                return True
            if Envariable.TASK_TYPE == 'clear':
                if Envariable.get_task_otype() == 'File' and self.lines > int(
                        quotaResp['UrlRemain']):
                    sys.exit("\n[error]：UrlRemain is not enough {0}".format(
                        quotaResp['UrlRemain']))
                elif Envariable.get_task_otype(
                ) == 'Directory' and self.lines > int(quotaResp['DirRemain']):
                    sys.exit("\n[error]：DirRemain is not enough {0}".format(
                        quotaResp['DirRemain']))
                else:
                    return True
...
class doTask(object):
    def urlencode_pl(inputs_str):
        ...
        for index_i in range(0, len_str):
            ...
            if (chs >= 'A' and chs <= 'Z') or (chs >= 'a' and chs <= 'z') or (
                    chs >= '0' and chs <= '9') or (chs == ":") or (chs == "/"):
                if result_end == "":
                    result_end = chs
                else:
                    result_end += chs
...
class Refresh(object):
    def main(self, argv):
        ...
        try:
            if not (Envariable.get_ak() and Envariable.get_sk()
                    and Envariable.get_fd() and Envariable.get_task_type()):
                sys.exit(
